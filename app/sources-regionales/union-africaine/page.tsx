...
import { ChevronRight, FileText, Clock, BookOpen, ChevronLeft, Search } from "lucide-react"
...
export default function UnionAfricainePage() {
  ...
  useEffect(() => {
    const fetchUnionAfricaineDocuments = async () => {
      setLoading(true)
      try {
        const params = new URLSearchParams({
          category: "union-africaine",
        ...
        console.error("Erreur lors du chargement des documents Union Africaine:", error)
      } finally {
        setLoading(false)
      }
    }

    fetchUnionAfricaineDocuments()
  }, [])
  ...
  // Breadcrumb items pour Union Africaine
  const breadcrumbItems = [
    { label: "Accueil", href: "/" },
    { label: "Sources régionales", href: "/source-regional" },
    { label: "Union Africaine", isActive: true }
  ]
            ...
            <div className="flex-1 overflow-y-auto p-6">
              ...
              <div className="mb-8">
                ...
                <div className="space-y-2">
                  <Link
                    href="/sources-regionales/ohada"
                    className="w-full flex items-center justify-between px-3 py-2 rounded-md text-sm transition-all duration-200 text-gray-700 hover:bg-gray-100 hover:text-gray-900"
                  >
                    <span>OHADA</span>
                    <ChevronRight className="w-4 h-4" />
                  </Link>
                  <Link
                    href="/sources-regionales/union-africaine"
                    className="w-full flex items-center justify-between px-3 py-2 rounded-md text-sm transition-all duration-200 bg-blue-100 text-blue-700 shadow-sm"
                  >
                    <span>Union Africaine</span>
                    <ChevronRight className="w-4 h-4" />
                  </Link>
                  <Link
                    href="/sources-regionales/cemac"
                    className="w-full flex items-center justify-between px-3 py-2 rounded-md text-sm transition-all duration-200 text-gray-700 hover:bg-gray-100 hover:text-gray-900"
                  >
                    <span>CEMAC</span>
                    <ChevronRight className="w-4 h-4" />
                  ...
                  <Link
                    href="/sources-regionales/cedeao"
                    className="w-full flex items-center justify-between px-3 py-2 rounded-md text-sm transition-all duration-200 text-gray-700 hover:bg-gray-100 hover:text-gray-900"
                  >
                    <span>CEDEAO</span>
                    <ChevronRight className="w-4 h-4" />
              ...
              <div>
                <div className="space-y-2">
                  <Link
                    ...
                    className="w-full flex items-center justify-between px-3 py-2 rounded-md text-sm transition-all duration-200 text-gray-700 hover:bg-gray-100 hover:text-gray-900"
                  >
                    <span className="font-bold">Sources nationales</span>
                    <ChevronRight className="w-4 h-4" />
                  </Link>
                  <Link
                    ...
                    className="w-full flex items-center justify-between px-3 py-2 rounded-md text-sm transition-all duration-200 text-gray-700 hover:bg-gray-100 hover:text-gray-900"
                  >
                    <span className="font-bold">Sources internationales</span>
                    <ChevronRight className="w-4 h-4" />
                  </Link>
                </div>
              </div>
            ...
            <div className="bg-white border-b border-gray-200 px-4 lg:px-6 py-4 lg:py-6">
              <div className="flex flex-col space-y-2 lg:space-y-3">
                <div className="flex items-center text-sm text-gray-500">
                  <span className="text-gray-900 font-medium">Sources régionales</span>
                  <ChevronRight className="w-4 h-4 mx-2" />
                  <span className="text-gray-900 font-medium">Union Africaine</span>
                </div>
              ...
              {loading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-400 text-sm">
                    Aucun document de l'Union Africaine n'est disponible pour le moment.
                  </p>
                </div>
              ) : (
