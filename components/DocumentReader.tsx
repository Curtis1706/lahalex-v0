...
import { ChevronDown, ChevronRight, Search, Printer, Download, Share2, Menu, X, ChevronUp, Highlighter, Edit3, Bookmark } from 'lucide-react'
...
export function DocumentReader({ document, article, processedContent }: DocumentReaderProps) {
  ...
  useEffect(() => {
    ...
    if (article?.metadata?.id) {
      setTimeout(() => {
        const currentElement = ref.current?.querySelector(`[data-article-id="${article.metadata.id}"]`)
        if (currentElement) {
  ...
  const getColorByType = (type: string, level: number, isCurrentArticle: boolean) => {
    if (isCurrentArticle) return 'text-primary-lahalex font-semibold'
    if (type === 'article') return 'text-blue-600 hover:text-blue-700'

    switch (level) {
      case 1: return 'text-gray-900 font-semibold'
      case 2: return 'text-gray-800 font-medium'
      case 3: return 'text-gray-700 font-medium'
      case 4: return 'text-gray-700'
  ...
  const renderSection = (section: any, level = 0) => {
    ...
    const colorClass = getColorByType(section.type, section.level, isCurrentArticle)

    return (
      <div key={section.id} className="w-full">
        <div
          data-article-id={section.type === 'article' ? section.id : undefined}
          className={`flex items-start w-full px-3 py-2 text-sm transition-all duration-200 group ${isCurrentArticle ? '' : 'hover:bg-gray-50'}`}
        ...
        >
          ...
          {section.type === 'article' ? (
            <Link
              ...
              className={`flex-1 text-left leading-relaxed ${isCurrentArticle ? colorClass : ''}`}
              onClick={() => isMobile && setSidebarOpen(false)}
              title={section.title}
              style={isCurrentArticle ? undefined : { color: 'rgba(16, 130, 201, 1)' }}
            >
        ...
        {hasChildren && isExpanded && (
          <div className="border-l border-gray-200 ml-6">
            {section.children.map((childId: string) => {
              const childSection = document.structure.sections.find((s: any) => s.id === childId)
              return childSection ? renderSection(childSection, level + 1) : null
            })}
          </div>
        )}
      </div>
  ...
  const breadcrumbItems = [
    ...
    { label: getDocumentTypeLabel(document.type), href: `/documents?type=${document.type}` },
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="flex min-h-[calc(100vh-200px)]">
        {/* Sidebar Gauche */}
        <div className="hidden lg:block w-80 border-r border-gray-200 flex flex-col h-[calc(100vh-200px)] overflow-y-auto" style={{ backgroundColor: "#F8F3F4" }}>
          <div className="p-4 border-b border-gray-200 flex-shrink-0" style={{ backgroundColor: "#F8F3F4" }}>
        ...
        <div className="flex-1 min-w-0 overflow-y-auto h-[calc(100vh-200px)]">
          <div className="max-w-4xl mx-auto p-4 lg:p-8 bg-white" style={{ backgroundColor: "#FFFFFF" }}>
            <div className="mb-8">
            ...
            <article
              className="prose prose-lg max-w-none text-gray-900 leading-loose !text-justify font-sans"
              style={{
                wordBreak: 'break-word',
                fontFamily: '"SF Pro Text", "SF Pro Display", "SF Pro", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, "Noto Sans", sans-serif',
                fontWeight: 400,
                fontStyle: 'normal',
                fontSize: '17px',
                lineHeight: '30.1px',
    ...
    `}
        ...
        {/* Sidebar Droite */}
        <div className="hidden lg:block w-80 border-l border-gray-200 h-[calc(100vh-200px)] overflow-y-auto" style={{ backgroundColor: "#FFFFFF" }}>
          <div className="p-4 sticky top-0 border-b border-gray-200" style={{ backgroundColor: "#FFFFFF" }}>
            ...
            <div className="mb-6">
              <form onSubmit={handleDocumentSearchSubmit} className="relative">
                ...
                <Input
                  ...
                  style={{ backgroundColor: "rgba(255, 255, 255, 0.06)", borderColor: "#BCBCBC" }}
                />
            ...
            {/* Outils d'annotation */}
            <div className="space-y-2 rounded-[5px] p-2" style={{ backgroundColor: "rgba(250, 245, 239, 0.5)" }}>
              <div className="text-xs font-medium mb-1" style={{ color: "rgba(0,0,0,0.7)" }}>Outils d'annotation</div>
              <Button variant="outline" size="sm" className="w-full justify-start rounded-[5px]" style={{ backgroundColor: "rgba(238, 238, 238, 0)" }}>
          ...
          <>
            ...
            {sidebarOpen && (
              <div className="fixed inset-0 z-50">
                ...
                <div className="absolute left-0 top-0 h-full w-80 flex flex-col" style={{ backgroundColor: "#FFFFFF" }}>
                  <div className="p-4 border-b border-gray-200 flex items-center justify-between flex-shrink-0" style={{ backgroundColor: "#FFFFFF" }}>
                    <h2 className="text-lg font-bold text-gray-900">
                      {getDocumentTypeLabel(document.type)}
                    </h2>
                  ...
                  <div className="p-4 border-t border-gray-200 flex-shrink-0">
                    <form onSubmit={handleDocumentSearchSubmit} className="relative mb-4">
                      <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4" />
                      <Input
                        type="text"
                        placeholder="Rechercher dans le document"
