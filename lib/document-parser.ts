...
export class DocumentParser {
  ...
  static parseDocument(content: string, documentId: string, title: string): ParsedDocument {
    ...
    // Détecter si c'est une fiche de synthèse
    if (this.isFicheSynthese(content)) {
      return this.parseFicheSynthese(content, documentId, title)
    }
...
currentArticleContent = (hierarchyMatch.title ? hierarchyMatch.title.replace(/^[:\s]+/, '') : '') + '\n'
  ...
  private static detectDocumentType(title: string): 'constitution' | 'code' | 'loi' | 'decret' | 'fiche-synthese' | 'other' {
    ...
    if (titleLower.includes('fiche') || titleLower.includes('synthèse') || titleLower.includes('synthese')) return 'fiche-synthese'
    return 'other'
  }

  private static isFicheSynthese(content: string): boolean {
    // Détection basée sur le contenu et la structure
    const lines = content.split('\n').slice(0, 20) // Vérifier les 20 premières lignes
    const contentText = lines.join(' ').toLowerCase()
    
    // Vérifier si c'est une fiche de synthèse
    if (contentText.includes('fiche de synthèse') || contentText.includes('fiche synthèse')) {
      return true
    }
    
    // Vérifier si le contenu a une structure de fiche explicative
    // (pas de hiérarchie LIVRE/TITRE/CHAPITRE, mais des questions/réponses)
    const hasHierarchy = lines.some(line => 
      /^\*\*(LIVRE|TITRE|CHAPITRE|SECTION)\s+\d+/.test(line.trim())
    )
    
    // Détecter les questions avec "?" ou les titres de section numérotés
    const hasQuestions = lines.some(line => {
      const trimmed = line.trim()
      return (
        /[?]$/.test(trimmed) || // Ligne se terminant par "?"
        /^\d+\.\s+[A-Z]/.test(trimmed) || // Numérotation 1. 2. etc.
        /^[A-Z][^.!?]*$/.test(trimmed) && trimmed.length > 10 && trimmed.length < 200 // Titre en majuscules
      )
    })
    
    // Détection plus permissive : si le contenu contient des questions claires
    const hasClearQuestions = lines.some(line => {
      const trimmed = line.trim()
      return (
        trimmed.includes('?') || // Contient un point d'interrogation
        /^[A-Z][^.!?]*$/.test(trimmed) && trimmed.length > 15 && trimmed.length < 150 // Titre en majuscules
      )
    })
    
    // Si pas de hiérarchie mais des questions, c'est probablement une fiche de synthèse
    const isFiche = !hasHierarchy && (hasQuestions || hasClearQuestions)
    
    console.log(`🔍 Détection fiche de synthèse:`, {
      hasHierarchy,
      hasQuestions,
      hasClearQuestions,
      isFiche,
  ...
  private static parseFicheSynthese(content: string, documentId: string, title: string): ParsedDocument {
    console.log(`🔍 Parsing fiche de synthèse: ${title}`)
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Détecter les questions ou titres de section
      const questionMatch = this.matchQuestionOrTitle(line)
      ...
      if (questionMatch) {
        ...
        // Créer le nouvel article
        const articleId = this.generateFicheArticleId(questionMatch.title, order)
        const currentPath = currentSection ? [currentSection.id] : ['section-principale']
        
        currentArticleMetadata = {
          id: articleId,
          title: questionMatch.title,
          numero: `section-${order}`,
          document: documentId,
          path: [...currentPath],
          navigation: {
            parent: currentSection?.id || 'section-principale'
          }
        }
        ...
        // Créer la section pour cet article
        const articleSection: DocumentSection = {
          id: articleId,
          title: questionMatch.title,
          type: 'section-synthese',
          level: 2,
          order: order++,
          path: [...currentPath, articleId],
          children: []
        }
        
        sections.push(articleSection)
        
        // Ajouter comme enfant de la section principale
        if (currentSection) {
          currentSection.children.push(articleId)
        }
        
        console.log(`📄 Section fiche: ${articleId} - "${questionMatch.title}"`)
      } else if (currentArticleMetadata) {
        currentArticleContent += line + '\n'
      }
    }
    ...
    console.log(`🎯 Fiche de synthèse: ${articles.length} sections, ${sections.length} éléments`)
    ...
    return {
      metadata: {
        ...
        title,
        type: 'fiche-synthese',
        description: `Fiche de synthèse: ${title}`,
        structure: { sections, totalArticles: articles.length }
      },
      articles
    }
  ...
  private static matchQuestionOrTitle(line: string): { title: string } | null {
    // Détecter les questions ou titres de section dans une fiche de synthèse
    const trimmedLine = line.trim()
    
    // Titre principal (sans numérotation)
    if (/^[A-Z][^.!?]*$/.test(trimmedLine) && trimmedLine.length > 10 && trimmedLine.length < 200) {
      return { title: trimmedLine }
    }
    
    // Questions se terminant par "?"
    if (/[?]$/.test(trimmedLine) && trimmedLine.length > 10) {
      return { title: trimmedLine }
    }
    
    // Titres de section avec numérotation simple
    if (/^\d+\.\s+[A-Z]/.test(trimmedLine)) {
      return { title: trimmedLine }
    }
    
    // Titres en gras (markdown)
    if (/^\*\*[^*]+\*\*$/.test(trimmedLine)) {
  ...
  private static generateFicheArticleId(title: string, order: number): string {
    // Générer un ID unique basé sur le titre et l'ordre
    const cleanTitle = title.toLowerCase()
      .replace(/[^a-z0-9\s]/g, '')
      .replace(/\s+/g, '-')
      .substring(0, 50)
    return `fiche-${order}-${cleanTitle}`
  }
}
